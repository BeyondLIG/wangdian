...
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="${pageContext.request.contextPath}/admin/user/list
" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <!--
            <label>级别：</label><input type="text" name="vip" value="${vip}" class="form-control" size="10" />
            -->
            <label>状态：</label>
    ...
    <table class="table table-bordered table-hover table-striped table-top" data-selected-multi="true">
        ...
        <c:forEach var="list" items="${userList.content}" varStatus="i">
            <tr>
                <td>${i.index+1}</td>
                <td>${list.username}</td>
                <td><c:if test="${list.vip==0}">普通用户</c:if><c:if test="${list.vip==1}">VIP用户</c:if><c:if test="${list.vip==2}">店主</c:if></td>
                ...
                <td>
                    <a href="${pageContext.request.contextPath}/admin/user/add?id=${list.id}" class="btn btn-default" data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true">查看</a>
...
<script>
    var formUrl = "${pageContext.request.contextPath}/admin/user/list" + "?vip=${vip}";
