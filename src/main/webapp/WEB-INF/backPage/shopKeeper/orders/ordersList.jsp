...
<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="${pageContext.request.contextPath}/admin/shopKeeper/orders/list" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <%--<label>商品名称：</label><input type="text" name="shopName" value="${shopName}" class="form-control" size="9" />--%>
            ...
            <%--<div class="pull-right">--%>
                <%--<div class="btn-group">--%>
                     <%--<button type="button" class="btn-default" ><a href="${pageContext.request.contextPath}/admin/orders/add" data-toggle="dialog" data-width="900" data-height="500" data-id="dialog-mask" data-mask="true" style="text-decoration: none;"><i class="fa fa-plus"></i> 添加订单</a></button>--%>
                <%--</div>--%>
            <%--</div>--%>
        </div>
    </form>
...
<div class="bjui-pageContent">
    ...
    <table class="table table-bordered table-hover table-striped table-top" data-selected-multi="true">
        ...
        <tr>
            ...
            <%--<th data-order-field="shopNumber" >商品编号</th>--%>
            <%--<th data-order-field="shopName" >商品名称</th>--%>
        ...
        <c:forEach var="list" items="${ordersList.content}" varStatus="i">
            <tr>
                ...
                <%--<td>${list.shopNumber}</td>--%>
                <%--<td>${list.shopName}</td>--%>
                <td>${list.shopKeeper}</td>
                <td>${list.shopOrderMan}</td>
                <td>${list.telephone}</td>
                <td>${list.shopOrderTimeToString()}</td>
                <td><c:if test="${list.status==0}">提交订单</c:if><c:if test="${list.status==1}">已付款未发货</c:if><c:if test="${list.status==2}">已付款已发货</c:if><c:if test="${list.status==3}">已确认收货</c:if><c:if test="${list.status==4}">申请退款中</c:if><c:if test="${list.status==5}">已退款</c:if></td>
                <td>
                    <a href="${pageContext.request.contextPath}/admin/shopKeeper/orders/add?id=${list.id}" class="btn btn-default" data-toggle="dialog" data-width="900" data-height="560" data-id="dialog-mask" data-mask="true">查看详情</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
